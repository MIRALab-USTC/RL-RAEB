...
plt.style.use('ggplot') # plot style setting
import matplotlib
matplotlib.use('Agg')
...
colors = [".    ", "darkorange", "lightseagreen", "cornflowerblue", "orchid", "gray", "darkseagreen", "goldenrod", "darkorange", "mediumorchid", "darkturquoise" ]
COLORS = dict()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_dir", type=str, default=r"D:\Disciplines\MIRA\Research\NIPS2020\Code\plots")
    ...
    fig = plt.figure(figsize=(options.w,options.h)) # 创建一张图 figure 指定大小
    gs = GridSpec(6, 4)
    ax1 = plt.subplot(gs[:3, :2]) # 在figure中创建子图
    ...
    for env_name in os.listdir(log_dir):
        ...
        if not os.path.isdir(env_dir):
            continue
        ax = axs[i]
        ax.xaxis.set_major_locator(plt.MaxNLocator(5)) # ax 表示坐标轴，设置坐标轴刻度，定位
        ax.ticklabel_format(style='sci', axis='x', scilimits=(0, 0)) # 指定x轴的数量级缩放
        sub_figure_name = env_name_to_figure_name(env_name) # env name 到 figure name
        ...
        for algo_name in os.listdir(env_dir):
            ...
            if algo_name in algo_dic:
                ...
                continue
            x, y_mean, y_std, max_y = get_data_from_algo_dir(algo_dir, algo_name, x_limit) # 获取数据均值和标准差
