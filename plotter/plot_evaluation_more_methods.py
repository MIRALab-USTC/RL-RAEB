...
XLimit = {
    'delivery_ant': 1e6,
    'delivery_car': 1e6,
    'delivery_cheetah': 1e6,
    'electric_ant': 1e6,
    'electric_cheetah': 1e6,
    'electric_car': 1e6
}
...
SAC_aym_per = {
    ...
    'cheetah': 12895,
    'humanoid': 4340,
    'invert_pendulum': 1000
}

LEGENDORDER = ["raeb", "simhash", "lagrangian"]
...
def env_name_to_figure_name(env_name):
    dic = {
        ...
        "delivery_cheetah": "Delivery Half-Cheetah",
        "delivery_car": "Delivery Mountain Car",
        "electric_car": "Electric Mountain Car",
        "electric_cheetah": "Electric Half-Cheetah",
...
def get_plot_data_from_single_experiment(file_name, algo_name):
    ...
    dic_y = {
        ...
        "simhash": "evaluation/Average Returns",
        "lagrangian": "evaluation/Average Returns"
    }
            
    dic_x_1 = {
        "raeb": "exploration/num steps total",
        "simhash": "exploration/num steps total",
        "lagrangian": "exploration/num steps total"
    }
...
def run_plot(args):
    ...
    gs = GridSpec(1, 4)
    ax1 = plt.subplot(gs[0, 0]) # 在figure中创建子图 # walker to 5 walker to 3
    ax2 = plt.subplot(gs[0, 1]) # cheetah to 4 cheetah to 5 
    ax3 = plt.subplot(gs[0, 2]) # humanoid to 2 
    ax4 = plt.subplot(gs[0, 3]) # hopper to 3 hopper to 4
    # ax5 = plt.subplot(gs[0, 4]) # invert to 6
    # ax6 = plt.subplot(gs[0, 5]) # ant to 1

    axs = [ax1, ax2, ax3, ax4]
    print(os.listdir(args.log_dir))

    for i, env_name in enumerate(EnvName):   
        env_dir = os.path.join(args.log_dir, env_name)
        if env_name == "with_done" or "cheetah" in env_name:
            continue
        if os.path.exists(env_dir):
            ...
            for alg_name in os.listdir(env_dir):
                if alg_name == 'mbpo':
                    ...
                    if alg_name == "ppo" or alg_name == "rnd" or alg_name == "raeb_bad" or alg_name == "surprise_bad" or alg_name == "surprise_vision_int005":
                        print(alg_name)
                        continue
                    if alg_name == "asm_per":
                        x = np.arange(XLimit[env_name], step=1000)
                        y = np.ones_like(x) * SAC_aym_per[env_name]
                        p = ax.plot(x, y, linewidth=LINEWIDTH, color=get_color(alg_name), linestyle='--')
                        legend_dict[alg_name] = p[0]
                        continue
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log_dir", type=str, default="/home/zhwang/ICML_TO_IJCAI/data/IJCAI22_rebuttal/more_comparsion_methods")
    ...
    parser.add_argument("--fig_name", type=str, default="comparison_evaluation_more_methods.pdf")
    parser.add_argument("--n_data", type=int, default=100)
    parser.add_argument("--w", type=float, default=53)
