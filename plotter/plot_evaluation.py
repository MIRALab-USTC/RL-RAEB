...
XLimit = {
    'goal4': 1e6,
    'resource_cheetah_goal4': 1e6,
    'resource_mountaincar_10': 1e6,
    'car_fuel': 1e6,
    'cheetah_fuel': 1e6,
    'ant_fuel': 1e6
}
...
LEGENDORDER = ["surprise_vision", "surprise", "sac", "information_gain"]
...
def env_name_to_figure_name(env_name):
    dic = {
        "goal4": "Delivery Ant",
        "resource_cheetah_goal4": "Delivery Half-Cheetah",
        "resource_mountaincar_10": "Delivery Mountain Car",
        "car_fuel": "Electric Mountain Car",
        "cheetah_fuel": "Electric Half-Cheetah",
        "ant_fuel": "Electric Ant"
    }
    return dic[env_name] if env_name in dic else env_name

def alg_name_to_legend(alg_name):
    dic = {
        "surprise_vision": "RAEB (ours)",
        "surprise": "Surprise",
        "sac": "SAC",
...
def run_plot(args):
    ...
    gs = GridSpec(2, 3)
    ax1 = plt.subplot(gs[0, 1]) # 在figure中创建子图 # walker to 5 walker to 3
    ax2 = plt.subplot(gs[0, 0]) # cheetah to 4 cheetah to 5 
    ax3 = plt.subplot(gs[1, 2]) # humanoid to 2 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log_dir", type=str, default="/home/zhwang/research/ICML2021_Finaldata/final_data_211226/evaluation")
    ...
    parser.add_argument("--fig_name", type=str, default="comparison_evaluation_final.pdf")
