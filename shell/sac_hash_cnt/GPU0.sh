...
CUDA_VISIBLE_DEVICES=0 nohup python scripts/run.py configs/sac/sac_hash/beta_100_gamma_1/sac_with_hash_sqrt_k16.json > o_maze_hash_state_beta100_gamma_1.txt 2>&1 &

CUDA_VISIBLE_DEVICES=1 nohup python scripts/run.py configs/surprise-based/resource-augmented/comparsion/modelbased_sac_surprise_ant_maze_discount_0.999.json > o_surprise_0.999.txt 2>&1 &
