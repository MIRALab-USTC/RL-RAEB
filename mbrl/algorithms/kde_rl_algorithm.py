...
class KDEBatchRLAlgorithm(RLAlgorithm):
    def __init__(
            self,
            num_epochs,
            batch_size,
    ...
    def _before_train(self):
        ...
        if hasattr(self, 'init_expl_policy'):
            ...
            self._sample(self.min_num_steps_before_training)
        # 在train 之前 训练一个density model
        data = self.pool.dataset['observations']
        self.trainer.reward_fn.update(data)
        self.end_epoch(-1)
        ...
        for i in range(max(self.num_train_loops_per_epoch, self.num_train_models_per_epoch)):
            ...
            for _ in range(self.num_trains_per_train_loop):
                ...
                if i % self.train_model_freq == 0:
                    # 每次train model 的同时也更新reward function
                    data = self.pool.dataset['observations']
                    self.trainer.reward_fn.update(data)
                    # train model 
