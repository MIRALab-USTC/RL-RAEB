...
class ModelBasedBatchRLAlgorithm(RLAlgorithm):
    ...
    def _train_epoch(self, epoch):
        ...
        if self.num_train_models_per_epoch > self.num_train_loops_per_epoch:
            train_policy_freq = self.num_train_models_per_epoch / self.num_train_loops_per_epoch
            for i in range(self.num_train_models_per_epoch):
                self._sample(self.num_expl_steps_per_train_loop)
                gt.stamp('exploration sampling', unique=False)
                self.training_mode(True)
                for _ in range(self.num_trains_per_train_loop):
                    progress.update()
                    train_data = self.pool.random_batch(self.batch_size)
                    if i % train_policy_freq == 0:
                        params = self.trainer.train(train_data, 'both')
                    else:
                        params = self.trainer.train(train_data, 'model')
                    progress.set_description(format_for_process(params))
                gt.stamp('training', unique=False)
                self.training_mode(False)
            self.eval_collector.collect_new_paths(
            ...
            train_model_freq = self.num_train_loops_per_epoch / self.num_train_models_per_epoch
            for i in range(self.num_train_loops_per_epoch):
                ...
                for _ in range(self.num_trains_per_train_loop):
                    progress.update()
                    train_data = self.pool.random_batch(self.batch_size)
                    if train_model_freq % (i+1) == 0:
                        params = self.trainer.train(train_data, 'both')
                    else:
                        params = self.trainer.train(train_data, 'policy_value')
                    progress.set_description(format_for_process(params))
                gt.stamp('training', unique=False)
                self.training_mode(False)
            self.eval_collector.collect_new_paths(
