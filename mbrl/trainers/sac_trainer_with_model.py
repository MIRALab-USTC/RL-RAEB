...
class ModelBasedSACTrainer(SACTrainer):
    def __init__(
            self,
            env,
            policy,
    ...
    def reward_function_surprise(self, obs, actions, next_obs, rewards):
        ...
        if self.model.ensemble_size == 1:
            ...
            rewards += rewards_int
            return rewards, rewards_int
        else:
            raise NotImplementedError
    def train(self, data, train_mode):
        return self.train_from_numpy_batch(data, train_mode)

    def train_from_numpy_batch(self, np_batch, train_mode):
        self._num_train_steps += 1
        batch = ptu.np_to_pytorch_batch(np_batch)
        return self.train_from_torch_batch(batch, train_mode)

    def train_from_torch_batch(self, batch, train_mode):
        ...
        if train_mode == "both":
            # shaping reward
            rewards, rewards_int = self.reward_function_surprise(obs, actions, next_obs, rewards)
