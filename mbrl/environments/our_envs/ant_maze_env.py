...
from mbrl.environments.our_envs.ant_maze_hrl import maze_env_utils
from mbrl.environments.our_envs.ant_maze_hrl.ant_hrl import AntEnv

# Directory that contains mujoco xml files.
# MODEL_DIR = 'environments/assets'
MODEL_DIR = 'mbrl/environments/our_envs/assets'
...
class MazeEnv(gym.Env, utils.EzPickle):
  ...
  def __init__(
    ...
    if self.cargo_num:
      self.wrapped_env = model_cls(*args, file_path=file_path,cargo_num=self.cargo_num, **kwargs)
    else:
  ...
  def _judge_goal_range(self, x, y, goal=None):  # this only finds one goal!
    if goal is None:
  ...
  def _get_obs(self):
    ...
    if self.step_count:
      ...
      return np.concatenate([wrapped_obs,
                            range_sensor_obs.flat] + view)

  def reset(self):
    self.t = 0
    self.trajectory = []
    self.wrapped_env.reset()
    self.wrapped_env.cargo = self.cargo_num
  ...
  def step(self, action):
    ...
    if self.cargo_num is not None:
      ...
      inner_reward = 0
      # if row == 3 and col == 1:
      if self._judge_goal_range(robot_x, robot_y):
        if not(row == self.goal_position[0] and col == self.goal_position[1]):
          print('*'*60)
          print('error')
          print('*'*60)
        inner_reward = 1000 * info['cargo_action']
        if self.wrapped_env.cargo < 1e-6:
          done = True
