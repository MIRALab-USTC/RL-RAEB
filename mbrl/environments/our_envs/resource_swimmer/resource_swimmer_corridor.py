...
class SwimmerCorridor(SwimmerEnv):
    def __init__(self, reward_block):
        self.reward_block = reward_block
        #HalfCheetahEnv.__init__(self)
        dir_path = os.path.dirname(os.path.realpath(__file__))
        mujoco_env.MujocoEnv.__init__(self, '%s/swimmer_corridor.xml' % dir_path, 5)
    ...
    def step(self, action):
        ...
        if xposafter >= self.reward_block[0]:
            reward = 100
            done = True
...
if __name__=='__main__':
    ...
    print(dire)
    video_env = VideoEnv("swimmer_corridor_v0", directory=dire)
