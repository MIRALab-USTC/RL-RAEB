...
class SimplePoolWithHash(SimplePool):
    ...
    def add_samples(self, samples):
        ...
        for k in self.fields:
            if k == "rewards":
                # add reward bonus
                v = self._update_rewards(states, samples[k], self.beta)
            else:
                v = samples[k]
    ...
    def _update_rewards(self, states, rewards, beta):
        # states shape (batch_size, dim_state)
        batch_phi = torch.matmul(torch.FloatTensor(states).to(self.device), torch.t(self.A))
        ...
        reward_bonus_count = np.reshape(reward_bonus_count, rewards.shape)
        rewards = rewards + beta/(reward_bonus_count+1)
        return rewards
    ...
    def _compute_batch_count(self, encode_batch_phi):
        virtual_count = np.zeros(encode_batch_phi.shape[0])
        for i in range(encode_batch_phi.shape[0]):
            if encode_batch_phi[i].tostring() in self.hash_table.keys():
                virtual_count[i] = self.hash_table[encode_batch_phi[i].tostring()]
